...
export default function RoleManager({ isOpen, onToggle, isMobile }) {
  ...
  const innerContent = (
    <>
      ...
      <div className={isOpen ? 'flex flex-col flex-1 min-h-0' : 'hidden'}>
        {/* Stats section */}
        <div className="px-3 py-2 border-b border-gray-100 space-y-1 flex-shrink-0">
  ...
  if (isMobile) {
    return (
      <>
        ...
        {showModal && (
          <AddRoleModal onAdd={addRole} onClose={() => setShowModal(false)} />
        )}
      </>
    )
  }
